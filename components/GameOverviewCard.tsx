import { GameCard } from "@/components/GameCard";
import { Play, Pause, Square, Edit } from "lucide-react";

const GameOverviewCard = ({ games, onUpdate, onEdit, onStatusChange, userId, icons, setIsAddGameModalOpen }) => {
  return (
    <div className="col-span-2 bg-white p-4 rounded-lg shadow">
      <h2 className="text-2xl font-semibold mb-4 text-gray-800">Games Overview</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {games.slice(0, 4).map((game) => (
          <GameCard
            key={game.id}
            game={game}
            onUpdate={onUpdate}
            onEdit={() => onEdit(game)}
            onStatusChange={(newStatus) => onStatusChange(game.id, newStatus)}
            userId={userId}
            icons={icons} // Pass icons as props
          />
        ))}
      </div>
      <button
        className="mt-4 bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"
        onClick={() => setIsAddGameModalOpen(true)} // Ensure this function is passed
      >
        Add New Game
      </button>
    </div>
  );
};

export default GameOverviewCard;